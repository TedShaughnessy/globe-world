...
The source jars below are generated by Loom. If either source jar is absent,
ask the user to run the matching task from the repository root:

- Common/server sources: `./gradlew :mod-fabric:genCommonSources`
- Client-only sources: `./gradlew :mod-fabric:genClientOnlySources`
- Both common and client-only sources: `./gradlew :mod-fabric:genSources`

...
Delayed replay now uses a compare-before-set guard: each spillover write stores
the block state that was present when the wrapped write was accepted, and
`WorldGenSpillover.applyToChunk(...)` only replays it if the canonical
destination still has that state. This is intended to prevent late replay of
soft decoration such as leaves or short grass from overwriting trunks or other
blocks generated by the destination chunk.
...
- Queued writes carry their expected replaced state and skip replay if the
  destination changed before application.

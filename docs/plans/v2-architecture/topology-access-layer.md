...
distances. It also owns `shouldAllowAliasMutation` for the current server-side
alias mutation guard. Server chunk lookup, block mutation/block-entity access,
canonical alias tickets, full chunk packet relabeling, block packet fanout,
biome resend fanout, world-event block fanout, tick canonicalization, spawning
chunk collection, chunk player provider checks, client mutation guards, and
bulk section access call those names at their subsystem boundaries.

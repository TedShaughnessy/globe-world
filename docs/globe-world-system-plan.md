...
| Scheduled block/fluid ticks | Mostly done | Scheduled ticks may be queued at raw alias positions or duplicated across aliases. | Canonicalize `LevelTicks.schedule`, `hasScheduledTick`, and `willTickThisTick` positions so queued ticks store and execute in canonical block coords. Clone/area tick operations still need edge-case testing. |

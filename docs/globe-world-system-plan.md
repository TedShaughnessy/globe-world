...
| Scheduled block/fluid ticks | Done for gameplay path | Scheduled ticks may be queued at raw alias positions or duplicated across aliases. | Canonicalize `LevelTicks.schedule`, `hasScheduledTick`, and `willTickThisTick` positions so queued ticks store and execute in canonical block coords. Verified with water flow. Clone/area tick operations still need edge-case testing. |
